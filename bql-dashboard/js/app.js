/**
 * BQL Smart Trash Bin System - App Entry Point & Controller (Senior Architecture)
 * Glues Map, Sidebar Stats Manager, and Theme Controller together
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderRoutePanel(routeResult) {
    ...
    if (!panel) {
      ...
      const viewWrapper = document.querySelector(".map-view-wrapper");
      if (viewWrapper) viewWrapper.appendChild(panel);
    }
    ...
    panel.innerHTML = `
      <div class="route-modal-header">
        ...
        <button class="tb-btn" onclick="window.clearRouteOnMap()" style="color: var(--text-muted);">✕</button>
      </div>
        ...
        ${routeResult.route.map((step, idx) => {
          ...
          return `
            <div class="route-step-card" onclick="window.selectBinFromList('${step.id}')" style="cursor: pointer;">
              <div class="route-step-num" style="${isDepot ? 'background: var(--accent-cyan);' : ''}">${isDepot ? (idx === 0 ? 'S' : 'E') : idx}</div>
  ...
  window.dispatchCollectionRequest = function(binId) {
    if (binId === 'ROUTE_BATCH') {
      showToast(`🟢 Đã điều xe thu gom tự động theo tuyến đường tối ưu!`);
      return;
    }
...
/**
 * Theme Switcher Manager (Light Green/White & Dark Emerald)
 */
function initThemeManager() {
  ...
  const savedTheme = localStorage.getItem("bql_theme") || "light";
  setTheme(savedTheme);
  ...
  if (themeToggleBtn) {
    themeToggleBtn.addEventListener("click", () => {
      ...
      setTheme(newTheme);
      showToast(`🎨 Đã chuyển sang giao diện ${newTheme === "light" ? "Xanh Emerald & Trắng (Light)" : "Tối (Dark Emerald)"}`);
    });
  }

  function setTheme(theme) {
    document.documentElement.setAttribute("data-theme", theme);
    localStorage.setItem("bql_theme", theme);
...
function showToast(message) {
  ...
  toast.innerHTML = `
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--primary-green)" stroke-width="2.5">
