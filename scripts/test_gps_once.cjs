...
async function run() {
  ...
  for (const b of buttons) {
    ...
    if (text.includes('Bỏ qua') || text.includes('Tôi đã có tài khoản')) {
      await b.click();
      break;
    }
  }
  ...
  console.log('3️⃣ Bấm đăng nhập 1 chạm vai trò Cư dân...');
  const loginBtns = await page.$$('button');
  for (const b of loginBtns) {
    const text = await page.evaluate(el => el.innerText, b);
    if (text.includes('Cư dân') || text.includes('resident@demo.vn')) {
      await b.click();
      break;
    }
  }
